...
export function userAccountRiskParameterUpdate(app, simSocket, liveSocket) {
    /**
     * Call the `/userAccountRiskParameter/update` endpoint.
     * 
     ...
     */
    return async function userAccountRiskParameterUpdate({ body, env }) {
        const { accessToken } = app.Storage.getAuthResponseData();

        const headers = getHeaders(accessToken);
        const endpoint = '/userAccountRiskParameter/update';
